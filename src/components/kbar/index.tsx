...
export default function KBar({ children }: { children: React.ReactNode }) {
  ...
  const [_, setTab] = useLocalStorage(`supermail-tab`, "inbox");
  // const [threadId, setThreadId] = useThread()
  const [done, setDone] = useLocalStorage("supermail-done", false);
          ...
          <KBarAnimator className="text-foreground relative !mt-64 w-full max-w-[600px] !-translate-y-12 overflow-hidden rounded-lg border bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
            <div className="bg-white dark:bg-gray-800">
              <div className="border-x-0 border-b-2 dark:border-gray-700">
                <KBarSearch
                  defaultPlaceholder="Lütfen aramak istediğiniz kelimeyi giriniz..."
                  className="w-full border-none bg-white px-6 py-4 text-lg outline-none focus:ring-0 focus:ring-offset-0 focus:outline-none dark:bg-gray-800"
                />
              </div>
              <RenderResults />
            </div>
